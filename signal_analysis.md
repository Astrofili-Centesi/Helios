...
---
layout: default
title: Astrofili Centesi - Sistema Helios - Analisi dei segnali
---

<h1>Astrofili Centesi - Sistema Helios - Analisi dei segnali</h1>
    ...
    return colorMap[signalName];
        ...
        function updatePlot() {
            try {
                ...
                if (isNaN(n_hours) || isNaN(smooth_window) || isNaN(prominence) || isNaN(correlation_threshold) || isNaN(correlation_window) || isNaN(min_channels) || isNaN(short_window) || isNaN(long_window)) {
                    console.error('Error: Invalid input value found');
                    return;
                }
...
function plotGroupZooms(groups,processedData) {
    ...
    for (let i = 0; i < n_groups; i++) {
        ...
        // Compute a centered moving average of the given values
function centeredMovingAverage(values, windowSize) {
    ...
    for (let i = 0; i < values.length; i++) {
        ...
        const end = Math.min(values.length - 1, i + halfWindowSize);
        let sum = 0;
